...
export default function AddSpotForm({ tripId }: { tripId: number }) {
  ...
  async function handleSubmit() {
    if (!name.trim()) { setError('スポット名は必須です'); setName(''); return; }
    if (name.trim().length > 100) { setError('スポット名は100文字以内で入力してください'); setName(''); return; }
    if (category.trim().length > 50) { setError('カテゴリは50文字以内で入力してください'); setCategory(''); return; }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col gap-3">
      ...
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="スポット名 *" maxLength={100} className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-400" />
      <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="カテゴリ（例：観光、グルメ）" maxLength={50} className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-400" />
